...
class CommentCreateView(generics.CreateAPIView):
    def post(self, request, pk):
        ...
        try:
            ...
            return Response(data={"error": "'content' is required"},
                            status=status.HTTP_400_BAD_REQUEST)
        user=User.objects.get(pk=request.user.id)
        Comment.objects.create(author=user, post=post, content=content)
        return Response(data={"response": "successfully commented"},
...
class CommentGetView(generics.CreateAPIView):
    def get(self, request, pk):
        try:
            ...
            return Response(data={"error": "comment doesn't exist"},
                            status=status.HTTP_404_NOT_FOUND)
        return Response(CommentDetailSerializer(queryset).data,
                            status=status.HTTP_201_CREATED)
    permission_classes = (IsAuthenticated, )
...
class CommentDeleteView(generics.CreateAPIView):
    def delete(self, request, pk):
        ...
        if comment_obj.author.id == request.user.id:
            ...
            return Response(data={'error': "you can't delete this comment"},
        ...
        if comment_obj.author.id == request.user.id:
            try:
                ...
                return Response(data={"error": "'content' is required"},
                                status=status.HTTP_400_BAD_REQUEST)
            comment_obj.save()
            return Response(data={"response": "successfully updated"},
                            status=status.HTTP_201_CREATED)
        else:
            return Response(data={'error': "you can't update this comment"},
                            status=status.HTTP_403_FORBIDDEN)
    permission_classes = (IsAuthenticated, )
